...
public class RequestController {
	...
	ModelAndView attr1() {
		...
		mav.addObject("pname", "화룡");
		mav.addObject("jum", new int[] {78,89,100});
		mav.addObject("now", new Date());
		mav.addObject("per", new Person("강동원", "남", true));
		return mav;
	}
	...
	String attr2(HttpServletRequest request) {
		...
		request.setAttribute("pname", "화룡");
		request.setAttribute("jum", new int[] {11,22,33,44,55});
		request.setAttribute("now", new Date());
		request.setAttribute("per", new Person("고수", "남", true));
		return "req/attr";
	}
	...
	String attr3(Model mm) {
		...
		mm.addAttribute("pname", "정좌성");
		mm.addAttribute("jum", new int[] {99,77});
		mm.addAttribute("now", new Date());
		mm.addAttribute("per", new Person("장서건", "여", false));
		return "req/attr";
	}
	...
	String signup(Model mm) {
		return "req/signup";
	}
	
	@RequestMapping("signupReg")
	String signupReg( 
